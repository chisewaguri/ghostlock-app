...
pub struct FutexWaiter {
    pub local: u64,
    /// Negative byte depth below the syscall-entry sp.
    pub depth: i64,
    pub chain: String,
    pub frames: BTreeMap<String, u64>,
}
...
pub fn derive_futex_waiter(
    ...
    // 5.10's flat waiter has no wake_state, so the init store there is a plain
    // self-store and the cross-check below has nothing to confirm it against
