...
/* futex_init() sizes its buckets from num_possible_cpus(). _SC_NPROCESSORS_ONLN
 * reads low while a core is parked, and then no leak candidate can match the
 * kernel's bucket. */
...
void futex_init(void)
{
    ...
    /* futex_init rounds up to a power of two and indexes with
     * hash & (size - 1), so a non-power-of-two size would index past the end */
