 ...
 * too early and the fd_set has not been copied to the stack yet, too late and
 * the slot is reused. Either way the PI walk follows fake pointers that are no
 * longer valid. The waiter's /proc entry is the direct evidence it is parked
 * inside the syscall, so it is checked before the trigger fires. */
